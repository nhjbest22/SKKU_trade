document.addEventListener("DOMContentLoaded", () => {
  ...
  if (signupBtn) {
    signupBtn.addEventListener("click", () => {
      ...
      emailjs
        .send("service_i21m3w7", "template_6v67zai", {
          to_email: email,
          to_name: name,
  ...
  if (findIdBtn) {
    findIdBtn.addEventListener("click", () => {
      const emailPrefix = document
        .getElementById("find-email-prefix")
        .value.trim();
  ...
  if (findPasswordBtn) {
    findPasswordBtn.addEventListener("click", () => {
      ...
      if (
        !storedUser ||
        storedUser.id !== enteredId ||
        storedUser.email !== fullEmail
      ) {
      ...
      const maskedPassword =
        storedUser.password.substring(0, 3) +
        "*".repeat(storedUser.password.length - 3);

      emailjs
        .send("service_i21m3w7", "template_y6nkphb", {
          to_email: fullEmail,
